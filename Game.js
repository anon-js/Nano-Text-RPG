    ...
    module.exports = {
        ...
        info  : function() {
            ...
            return r.reply(
                ...
                "체력: " + user.s.hp + "/" + user.s.fhp + (user.s.die ? ((Date.now() - user.s.dieTime) < 180000 ? " (약 " + (Math.floor(((user.s.dieTime + 180000) - Date.now()) / 1000) >= 60 ? Math.floor((((user.s.dieTime + 180000) - Date.now()) / 1000) / 60) + "분" : Math.floor(((user.s.dieTime + 180000) - Date.now()) / 1000) + "초") + " 뒤 부활 가능)" : " (부활 가능)") : "") + "\n" +
                "공격력: " + user.s.dem + "\n" +
                "돈: " + user.s.money + "*\n" +
                "장소: " + user.s.place + "\n" +
                "\n" +
